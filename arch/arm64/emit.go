...
func (t *ARM64Target) Emit(f *ir.Function, globals []string) error {
	...
	for _, b := range f.Blocks {
		...
		for _, ins := range b.Ins { t.emitIns(ins, f) }
		t.emitJump(b, f)
	}
	return nil
}
...
func (t *ARM64Target) emitIns(ins ir.Instruction, f *ir.Function) {
	...
	case ir.Ocall: fmt.Fprintf(t.w(), "\tbl %s\n", t.formatRef(ins.Arg[0], ir.Kl, f))
	case ir.Oalloc4, ir.Oalloc8, ir.Oalloc16:
		off := int((ins.To.Val + 1) * 8)
		t.emitFrameAllocForReg(t.formatRef(ins.To, ir.Kl, f), off)
	case ir.Oceqw, ir.Oceql: t.emitCmp("eq", ins, f)
	case ir.Ocnew, ir.Ocnel: t.emitCmp("ne", ins, f)
	case ir.Ocsltw, ir.Ocsltl: t.emitCmp("lt", ins, f)
	case ir.Ocslew, ir.Ocslel: t.emitCmp("le", ins, f)
	case ir.Ocsgtw, ir.Ocsgtl: t.emitCmp("gt", ins, f)
	case ir.Ocsgew, ir.Ocsgel: t.emitCmp("ge", ins, f)
	case ir.Oneg:
		dst := t.formatRef(ins.To, ins.Cls, f)
